...
pub fn parse<'a> (src: &'a [u8], ts: &mut TokenIter) -> Result<NominalModule, Report> {
    ...
    match next.kind {
        TK::Eof => {},
        _ => { return error!(next.range, "No code after the module definition is allowed. This incident will be reported.") }
    }
    
    Ok(NominalModule { name: name, abstract_defs: ad, class_defs: cd, fun_defs: fd, expr: mexpr })
}

// Parses a sequence of 0 or more definitions.
// Examples: ``, `def f (): Unit := () end f`
fn parse_many_definitions<'a> (src: &'a [u8], ts: &mut TokenIter) -> Result<(VecDeque<NominalAbstDef>, VecDeque<NominalClassDef>, VecDeque<NominalFunDef>), Report> {
...
fn parse_pattern<'a> (src: &'a [u8], ts: &mut TokenIter) -> Result<Pattern<Name>, Report> {
    ...
    match tk.kind {
        ...
        TK::LitString => {
            let val = get_string(src, Span::new(tk.range.start + 1, tk.range.end - 1, tk.range.tag))?;
...
fn parse_simple_expr<'a> (src: &'a [u8], ts: &mut TokenIter) -> Result<Expr<Name>, Report> {
    ...
    match tk.kind {
        ...
        TK::LitString => get_string(src, Span::new(tk.range.start + 1, tk.range.end - 1, tk.range.tag)).map(|s| Expr::StringLiteral(s, tk.range)),
        TK::KwError => {
