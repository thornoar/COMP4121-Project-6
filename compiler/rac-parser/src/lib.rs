...
// pub fn parse<'a> (src: &'a [u8], tag: u8) -> Result<NominalModule, Report> {
//     let mut ts = TokenIter::new(src, src.len(), tag);
...
pub fn parse<'a>(src: &'a [u8], ts: &mut TokenIter) -> Result<NominalModule, Report> {
    ...
    if select!(src, id2.range) != select!(src, id1.range) {
        return error!(
            id2.range,
            "The names at the start and end of a module must match."
        );
    }
    ...
    match next.kind {
        ...
        _ => {
            return error!(
                next.range,
                "No code after the module definition is allowed. This incident will be reported."
            );
        }
    }
...
fn parse_many_arguments<'a>(
    src: &'a [u8],
    ts: &mut TokenIter,
) -> Result<ArgList<String, Name>, Report> {
    ...
    match delim.kind {
        ...
        _ => error!(
            delim.range,
            "Expected either a comma separator of a closing parenthesis"
        ),
    }
}
    ...
    match typ1.kind {
        TK::TypInt => {
            ...
            match str::from_utf8(&select!(src, size.range)) {
                ...
                _ => error!(
                    size.range,
                    "Only size `32` is supported for integer literals."
                ),
            }
        }
    ...
    match t1.kind {
        ...
        TK::KwIf => {
            ...
            expect!(
                ts,
                TK::KwEnd,
                "An `if` statement must terminate with `end if`"
            );
            let eif = expect!(
                ts,
                TK::KwIf,
            ...
            Ok(Expr::Ite(
                ...
                Box::new(if_branch),
                Box::new(else_branch),
                join(t1.range, eif.range),
            ))
        }
        _ => parse_with_match(src, ts),
    }
}
                ...
                loop {
                    ...
                    match tk.kind {
                        ...
                        _ => {
                            return error!(
                                tk.range,
                                "Expected either a `case` keyword or a closing curly bracket"
                            );
                        }
                    }
                }
...
fn parse_pattern<'a>(src: &'a [u8], ts: &mut TokenIter) -> Result<Pattern<Name>, Report> {
    ...
    match tk.kind {
        ...
        TK::LitString => {
            let val = get_string(
                src,
                Span::new(tk.range.start + 1, tk.range.end - 1, tk.range.tag),
            )?;
        ...
        TK::OpenParen => {
            ...
            match cp.kind {
                ...
                _ => error!(
                    cp.range,
                    "Expected a closing parenthesis to finish the unit literal pattern."
                ),
            }
        }
        TK::Identifier => {
            ...
            match next.kind {
                TK::OpenParen => {
                    ...
                    let cp = expect!(
                        ts,
                        TK::CloseParen,
                        "Expected a closing parenthesis at the end of a pattern list"
                    );
                    Ok(Pattern::ClassPattern(
                        qname,
                        argpats,
                        join(idrange, cp.range),
                    ))
                }
                _ => match qname.owner {
...
fn parse_pattern_list<'a>(
    src: &'a [u8],
    ts: &mut TokenIter,
) -> Result<VecDeque<Pattern<Name>>, Report> {
    match ts.peek().kind {
...
fn parse_infix_expr<'a>(
    src: &'a [u8],
    ts: &mut TokenIter,
    level: u8,
) -> Result<Expr<Name>, Report> {
    if level <= 0 {
...
fn parse_simple_expr<'a>(src: &'a [u8], ts: &mut TokenIter) -> Result<Expr<Name>, Report> {
    ...
    match tk.kind {
        ...
        TK::LitString => get_string(
            src,
            Span::new(tk.range.start + 1, tk.range.end - 1, tk.range.tag),
        )
        .map(|s| Expr::StringLiteral(s, tk.range)),
        TK::KwError => {
            expect!(
                ts,
                TK::OpenParen,
                "The `error` keyword must be followed by an open parenthesis"
            );
...
mod tests {
    ...
    fn test_parse_expr() {
        ...
        "
        .as_bytes();
        let mut ts = TokenIter::new(src, src.len(), 0);
